...
export const useServiceGroupStore = defineStore('service-group', () => {
  ...
  const setServiceGroup = data => {
    ...
    const localStorageSgName = localStorage.getItem(LOCALSTORAGE_KEY.SERVICE_GROUP_NAME)
    ...
    if (data.length && !localStorageSg) {
      ...
      if (findItem) {
        ...
        if (!localStorageSgName) {
          localStorage.setItem(LOCALSTORAGE_KEY.SERVICE_GROUP_NAME, findItem.groupName)
        }
      } else {
        // 목록에서 첫번째 아이템의 정보 매핑
        const topSg = data[0]
